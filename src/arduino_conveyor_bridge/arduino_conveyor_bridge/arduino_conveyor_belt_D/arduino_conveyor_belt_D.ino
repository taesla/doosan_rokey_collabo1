...
// ★ 추가: 직전 안정 상태 (상승 에지 검출용)
bool prevSensorStable = false;
// ---- DETECT 카운트 (로깅용 ID) ----
unsigned long detectCount = 0;
// ★ 추가: 바로 RESUME 후에 튀는 DETECT 막기 위한 타이머 (옵션)
...
void setup() {
  ...
  prevSensorStable = sensorStable;  // ★ 추가
  sensorChangeTime = millis();
  Serial.println("BOOT");  // 디버깅용
}
void loop() {
  // 1) 로봇 :양방향_화살표: 컨베이어 통신: 명령 수신 체크
  handleSerialCommand();
  // 2) 센서 상태 디바운싱
  ...
  if (nowMs - sensorChangeTime > 50) {
    sensorStable = raw;
  }
  // ★ 여기서 상승 에지 계산 (OFF → ON 되는 순간만 true)
  bool risingEdge = (!prevSensorStable && sensorStable);
  prevSensorStable = sensorStable;
  // 3) 아직 로봇을 기다리는 중이 아니고, 비상정지도 아닐 때만 센서 이벤트 처리
  if (!waitingForRobot && !emergencyStop) {
    // ★ RESUME 직후 100ms 정도는 무시 (옵션, 튐 방지)
    if (nowMs - lastResumeTime < 100) {
      // 너무 바로 직후면 DETECT 발생 안 함
    } else {
      if (risingEdge && !sentDetect) {
        // 물체가 새로 들어온 순간 (OFF → ON 으로 바뀐 순간)
        detectCount++;
        Serial.print("DETECT#");
