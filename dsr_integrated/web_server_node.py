...
# 복구용 상태 (Firebase 저장 및 재시작 시 복구용)
recovery_state = {
    # 시스템 정보
    'system_restart_count': 0,
    'last_crash_reason': None,      # 'COLLISION', 'DRIVER_CRASH', 'MANUAL_RESTART', None
    'last_crash_time': None,
    'recovery_attempts': 0,         # 현재 복구 시도 횟수 (최대 3)
    
    # 작업 상태 (상세)
    'last_action': 'idle',          # 'approaching_pick', 'picking', 'picked', 'approaching_place', 'placing', 'placed', 'returning_home', 'idle'
    'action_timestamp': None,
    'gripping': False,              # 물체 잡고 있는지
    
    # 위치 정보
    'z_touch': 153.0,               # 마지막 접촉 높이
    'target_place_position': None,  # Place 목표 위치 [x,y,z,rx,ry,rz]
    
    # 물체 정보
    'object_width_class': None,     # 'SMALL', 'MEDIUM', 'LONG'
...
class WebServerNode(Node):
    ...
    def __init__(self):
        ...
        # 복구 상태 토픽 구독 (sort_node에서 발행)
        self.recovery_state_sub = self.create_subscription(
            String,
            '/dlar/recovery_state',
            self.recovery_state_callback,
            10,
            callback_group=self.callback_group
        )
        ...
        # 드라이버 재시작 필요 토픽 구독 (외부 watcher 노드에서 발행)
        self.driver_restart_sub = self.create_subscription(
            Bool,
            '/dlar/driver_restart_needed',
            self.driver_restart_callback,
            10,
            callback_group=self.callback_group
        )
        self.get_logger().info('드라이버 재시작 토픽 구독 완료')
    ...
    def recovery_state_callback(self, msg):
        """복구 상태 콜백 (sort_node에서 발행)"""
        global recovery_state
        try:
            data = json.loads(msg.data)
            # 받은 데이터로 recovery_state 업데이트
            for key in ['last_action', 'action_timestamp', 'z_touch', 
                        'target_place_position', 'object_width_class', 'object_width_mm']:
                if key in data:
                    recovery_state[key] = data[key]
        except Exception as e:
            self.get_logger().debug(f'recovery_state 파싱 실패: {e}')
    ...
    def driver_restart_callback(self, msg):
        """드라이버 재시작 필요 콜백 (외부 watcher 노드에서 발행)"""
        global logs
        
        if msg.data:
            self.get_logger().error('🔴 드라이버 재시작 필요 신호 수신!')
            
            kst = timezone(timedelta(hours=9))
            log_entry = {
                'time': datetime.now(kst).strftime('%H:%M:%S'),
                'level': 'ERROR',
                'message': '드라이버 충돌 감지 - 재시작 필요'
            }
            logs.insert(0, log_entry)
            if len(logs) > 100:
                logs.pop()
            ...
            # 웹 UI에 알림
            socketio.emit('driver_crash', {
                'message': '드라이버 충돌 감지! 재시작이 필요합니다.',
                'timestamp': datetime.now(kst).isoformat()
            })
            
            # TODO: 자동 복구 로직 호출 (collision_recovery 또는 드라이버 재시작)
            # 현재는 알림만 보냄. 실제 복구는 사용자 확인 후 수동으로 진행
...
def firebase_upload_thread():
    ...
    global firebase_ref, ros_node, ui_state, logs, sort_status, recovery_state
    ...
    while True:
        try:
            if robot_data['connected']:
                ...
                # 그리퍼 상태 계산 (DO1=0, DO2=1 → 열림 / DO1=1, DO2=0 → 닫힘)
                do1 = (robot_data['controller_digital_output'] >> 0) & 1
                do2 = (robot_data['controller_digital_output'] >> 1) & 1
                gripping = (do1 == 1 and do2 == 0)  # 닫힘 = 물체 잡음
                recovery_state['gripping'] = gripping
                
                # 업로드할 데이터 (필수 정보 + 로그 + 분류 상태 + 복구 상태)
                upload_data = {
                    'timestamp': time.time(),
                    ...
                    # 복구용 상태 추가
                    'recovery_state': {
                        'system_restart_count': recovery_state.get('system_restart_count', 0),
                        'last_crash_reason': recovery_state.get('last_crash_reason'),
                        'last_crash_time': recovery_state.get('last_crash_time'),
                        'recovery_attempts': recovery_state.get('recovery_attempts', 0),
                        'last_action': recovery_state.get('last_action', 'idle'),
                        'action_timestamp': recovery_state.get('action_timestamp'),
                        'gripping': gripping,
                        'z_touch': recovery_state.get('z_touch', 153.0),
                        'target_place_position': recovery_state.get('target_place_position'),
                        'object_width_class': recovery_state.get('object_width_class'),
                        'object_width_mm': recovery_state.get('object_width_mm'),
                    },
                    # 최근 로그 추가
                    'logs': recent_logs
                }
